...
void Toolbar::layout_mode(Scene& scene)
{
    if (ImGui::BeginTabItem("Layout")) {
        ...
        */
            float Roll = radians(z_angle), Pitch = radians(x_angle), Yaw = radians(y_angle);
            Quaternionf quaternion = Quaternionf::Identity();
            Quaternionf Zquaternion (cos(Roll / 2), 0, 0, sin(Roll / 2));
            Quaternionf Xquaternion(cos(Pitch / 2), sin(Pitch / 2), 0, 0);
            Quaternionf Yquaternion(cos(Yaw / 2), 0, sin(Yaw/2), 0);
            quaternion = Xquaternion * Yquaternion * Zquaternion * quaternion;
