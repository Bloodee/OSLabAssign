 ...
 void Scene::simulation_update()
{
    ...
    std::chrono::steady_clock::time_point current_time = std::chrono::steady_clock::now();
     ...
     duration time_span =current_time - last_update;

     float total_simulation_time = time_span.count();

     size_t count = 0;

     while (total_simulation_time >= time_step) {
        for (auto object : all_objects) object->update(all_objects);
        total_simulation_time -= time_step;
        count++;
     }

     last_update =last_update + std::chrono::duration_cast<std::chrono::steady_clock::duration>(
                           (duration)count * time_step);
